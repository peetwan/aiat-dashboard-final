...
SOURCE_PROFILES: dict[str, dict[str, object]] = {
    ...
    "f2_target_household": {
        "what_we_use_th": "ข้อมูลที่หน้าเว็บสาธารณะแสดงสามารถใช้ตรวจในเครื่องได้ แต่ Dashboard Final ไม่ส่ง values ชุดนี้ขึ้น Cloud",
        "grain_th": "ข้อมูลครัวเรือน/เคสตามต้นทาง; เก็บเฉพาะ local lane",
        "dashboard_use_th": "แสดงเพียง metadata และสถานะ restricted",
        "excluded_th": "ค่า record ทั้งหมดไม่อยู่ใน Cloud Serving Database; ตัดชื่อ เบอร์โทร อีเมลเมื่อทำ extracted",
        "database_targets": ["sources"],
    },
    ...
    "f2_wallet_all_realtime": {
        "what_we_use_th": "Aggregate รายเดือนของกระเป๋าครัวเรือนและธุรกิจจาก public APIs สำหรับการตรวจในเครื่อง",
        "grain_th": "เดือน × ประเภทกระเป๋า × ตัวชี้วัด aggregate ไม่ใช่รายการธุรกรรมรายบุคคล",
        "dashboard_use_th": "Cloud แสดงเพียง metadata และสถานะ restricted",
        "excluded_th": "ไม่ส่ง values ขึ้น Cloud; snapshot ที่มีในเครื่องอาจไม่ครบทุกเดือนของ API",
        "database_targets": ["sources", "endpoints"],
    },
    "f2_wallet_cluster_realtime": {
        "what_we_use_th": "Aggregate รายเดือนเปรียบเทียบตามกลุ่มธุรกิจ/คลัสเตอร์จาก public APIs สำหรับการตรวจในเครื่อง",
        "grain_th": "เดือน × กลุ่มธุรกิจ × ประเภทครัวเรือน/ธุรกิจ × ตัวชี้วัด aggregate",
        "dashboard_use_th": "Cloud แสดงเพียง metadata และสถานะ restricted",
        "excluded_th": "ไม่ส่ง values ขึ้น Cloud และไม่เปิดเผยกลุ่มขนาดเล็กเป็นรายบุคคล",
        "database_targets": ["sources", "endpoints"],
    },
