...
`public_contact` ครอบคลุมข้อความที่รวมชื่อ อีเมล เบอร์ LINE/social และที่อยู่สำหรับติดต่องานไว้ในช่องเดียวด้วย ส่วน `public_location` ใช้เมื่อเป็นที่ตั้งอย่างเดียว ข้อความเช่น `LINE ID: work_account` หรือ `Facebook: @work_account` ในฟิลด์คำอธิบายต้องระบุบริบทที่ฟิลด์นั้นด้วย คำทั่วไปอย่างชื่อสายรถไฟและ URL อ้างอิงที่ไม่มีป้ายข้อมูลติดต่อยังคงใช้ได้
